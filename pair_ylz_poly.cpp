...
PairYLZPoly::PairYLZPoly(LAMMPS *lmp) :
...
void PairYLZPoly::compute(int eflag, int vflag)
{
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsq[itype][jtype]) {
        ...
        one_eng = ylz_poly_analytic(i, j, a1, a2, r12, rsq, fforce, ttor, rtor);
...
//add pressure, lambda term only used for i and j of same type， so only f[i] is modified here

  f[i][0] -= lambda[itype][itype] * a1[0][0];
  f[i][1] -= lambda[itype][itype] * a1[0][1];
  f[i][2] -= lambda[itype][itype] * a1[0][2];
}
  ...
  if (flag_all) error->all(FLERR, "All atoms for pair style ylz/poly must be ellipsoids");
}
...
void PairYLZPoly::settings(int narg, char **arg)
{
...
void PairYLZPoly::coeff(int narg, char **arg)
{
...
void PairYLZPoly::init_style()
{
  avec = dynamic_cast<AtomVecEllipsoid *>(atom->style_match("ellipsoid"));
  if (!avec) error->all(FLERR, "Pair style ylz/poly requires atom style ellipsoid");
...
double PairYLZPoly::init_one(int i, int j)
{
  if (setflag[i][j] == 0) {
...
void PairYLZPoly::write_restart(FILE *fp)
{
...
void PairYLZPoly::read_restart(FILE *fp)
{
...
void PairYLZPoly::write_restart_settings(FILE *fp)
{
...
void PairYLZPoly::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
void PairYLZPoly::write_data(FILE *fp)
{
  for (int i = 1; i <= atom->ntypes; i++)
...
void PairYLZPoly::write_data_all(FILE *fp)
{
  for (int i = 1; i <= atom->ntypes; i++)
    for (int j = i; j <= atom->ntypes; j++)
...
double PairYLZPoly::ylz_poly_analytic(const int i, const int j, double a1[3][3], double a2[3][3],
...

{
  ...
  double t, t2, t4, f_t, f1, U, uR, uA, dUdr, dfdr, dUdphi;
  ...
  const double Termi= sint - ni1rhat;
  const double Termj= sint + nj1rhat;


  const double a = ninj + Termi * Termj - 2.0 * sint * sint;
  const double phi = 1.0 + (a - 1.0) * muu;
  
  
  dphi_drhat[0] = muu * (Termi * nj1[0] - ni1[0] * Termj);
  dphi_drhat[1] = muu * (Termi * nj1[1] - ni1[1] * Termj);
  dphi_drhat[2] = muu * (Termi * nj1[2] - ni1[2] * Termj);

  dphi_dni1[0] = muu * (nj1[0] - r12hat[0] * Termj);
  dphi_dni1[1] = muu * (nj1[1] - r12hat[1] * Termj);
  dphi_dni1[2] = muu * (nj1[2] - r12hat[2] * Termj);

  dphi_dnj1[0] = muu * (ni1[0] + r12hat[0] * Termi);
  dphi_dnj1[1] = muu * (ni1[1] + r12hat[1] * Termi);
  dphi_dnj1[2] = muu * (ni1[2] + r12hat[2] * Termi);
  
  const double inv_L =1.0 / ( rcut - rmin );
  const double inv_L2 = inv_L * inv_L;
  const double l = 0.5 * (rcut + rmin);
  ...
  if (r < rmin) {
    ...
    t = ( r - l );
    t2 = t * t;
    f_t = ( 2.0 * inv_L2  * t2 - 1.5 )* inv_L * t + 0.5;
    f1 = f_t;
    dfdr = ( 6.0 * t2 * inv_L2 - 1.5) * inv_L;

    for (int k = 1; k <= zt - 2; k++) f1 *= f_t;    // get f()^(zt-1) zt>=2

    uA = -energy_well * f1 * f_t;
    U = uA * phi;
    dUdr =  - zt * (f1) *(dfdr) * phi * energy_well;
    dUdphi = uA;
  }
...
void *PairYLZPoly::extract(const char *str, int &dim)
{
  dim = 2;
